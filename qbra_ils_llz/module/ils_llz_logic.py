...
# This module encapsulates the logic of ILS LLZ single frequency,
# preserving the original calculations and geometry construction.
# - No direct use of iface/UI
# - Strictly maintain math/geometry outputs


def compute_parameters(navaid: str, a: float, d0: float) -> Tuple[float, float, float, float, float, float, float, float]:
    """
    Preserve original parameter selection exactly as legacy script.
    Returns (a, b, h, r, D, H, L, phi).
    """
    if navaid == 'LOC':
...
def _qpoint_with_z(pt_xy: QgsPointXY, z: float) -> QgsPoint:
    p = QgsPoint(pt_xy)
    p.addZValue()
    p.setZ(z)
    return p
...
def build_layers(p_geom: QgsPointXY, azimuth: float, site_elev: float, navaid: str, a: float, d0: float, map_srid: str, remark: str):
    """
    Create memory layers (PointZ and PolygonZ) with geometries exactly as in the legacy script.
    Returns (z_layer, v_layer).
    """
    ...
    def ring(points_with_z):
        return QgsLineString([_qpoint_with_z(QgsPointXY(p), z) for p, z in points_with_z])

    # Base
    base_pts = [(pt_bl,site_elev),(pt_br,site_elev),(pt_ar,site_elev),(pt_al,site_elev),(pt_bl,site_elev)]
    geom_poly = QgsPolygon(rings=[])
    geom_poly.setExteriorRing(ring(base_pts))
    seg = QgsFeature(); seg.setGeometry(QgsGeometry(geom_poly))
    ...
    geom_poly = QgsPolygon(); geom_poly.setExteriorRing(ring(llevel_pts))
    seg = QgsFeature(); seg.setGeometry(QgsGeometry(geom_poly))
    ...
    geom_poly = QgsPolygon(); geom_poly.setExteriorRing(ring(rlevel_pts))
    seg = QgsFeature(); seg.setGeometry(QgsGeometry(geom_poly))
    ...
    arc = QgsCircularString.fromTwoPointsAndCenter(
        _qpoint_with_z(QgsPointXY(pt_rl), site_elev + h),
        _qpoint_with_z(QgsPointXY(pt_rr), site_elev + h),
        _qpoint_with_z(QgsPointXY(p_geom), site_elev + h)
    )
    line_start = QgsLineString([
        _qpoint_with_z(QgsPointXY(pt_rr), site_elev + h),
        _qpoint_with_z(QgsPointXY(pt_ar), site_elev),
        _qpoint_with_z(QgsPointXY(pt_al), site_elev),
        _qpoint_with_z(QgsPointXY(pt_rl), site_elev + h),
    ])
    ...
    geom_poly = QgsPolygon(); geom_poly.setExteriorRing(ring(wall1_pts))
    seg = QgsFeature(); seg.setGeometry(QgsGeometry(geom_poly))
    ...
    geom_poly = QgsPolygon(); geom_poly.setExteriorRing(ring(wall2_pts))
    seg = QgsFeature(); seg.setGeometry(QgsGeometry(geom_poly))
    ...
    geom_poly = QgsPolygon(); geom_poly.setExteriorRing(ring(wall3_pts))
    seg = QgsFeature(); seg.setGeometry(QgsGeometry(geom_poly))
